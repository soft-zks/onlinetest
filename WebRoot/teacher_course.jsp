...
<body>
	<div class="container">
		<div class="besttop" style="height: 90px;">
			<h1 class='page-header' style="display: inline; line-height: 90px;">教师——查看所教课程</h1>
			...
			<table class='table table-striped table-bordered table-hover table-condensed'>
				...
				<c:forEach items="${teacherCourses}" var="course" varStatus="vs">
					<tr>
						<td>${course.cname}</td>
						<td>
							 <form id="myform" action="${pageContext.request.contextPath}/doProduce" method="post">
	     						<input type="hidden" value="教师" name="mytype"/>
	     						<input type="hidden" id="changeType" name="type"/>
	     						<input type="hidden" value="${course.cno}" name="cno"/>
	     						<input type="hidden" value="${course.cname}" name="cname"/>
	     						<input type="button" onclick="doAutoTest()" value="自动出卷" class="btn btn-default"/>
	     						<input type="button" onclick="doHandTest()" value="手动出卷" class="btn btn-default"/>
	     					</form> 
						</td>
					</tr>
				</c:forEach>
